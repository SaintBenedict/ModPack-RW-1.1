...
namespace VAEShrubland
{
    ...
    public static class Patch_WildAnimalSpawner
    {
        ...
        public static class manual_SpawnWildAnimalAt_predicate
        {
            ...
            private static bool EligibleToSpawn(bool original, PawnKindDef a, Map map)
            {
                // If the pawn requires a river, check that the map also has a river
                if (original)
                {
                    var thingDefExtension = a.race.GetModExtension<ThingDefExtension>() ?? ThingDefExtension.defaultValues;
                    if (thingDefExtension.requiresRiver)
                    {
                        return map.TileInfo.Rivers != null;
                    }
                }
                return original;
            }
